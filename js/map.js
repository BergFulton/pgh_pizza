...
var ViewModel = function() {
    ...
    self.filteredItems = ko.computed(function() {
        ...
        // Method 2: Use the arrayFilter utility function (for reference)

        // Loop over all Pizza objects
        /*var newArray = ko.utils.arrayFilter(self.pizzaList(), function(pizzaItem) {
            // if the title contains the query value
            if (pizzaItem.title.toLowerCase().indexOf(query) !== -1) {
                pizzaItem.marker.setVisible(true); // Show Map Marker
                return true; // Show Pizza object in list view
            } else {
                pizzaItem.marker.setVisible(false); // Hide Map Marker
                return false; // Hide Pizza object in list view (this line is not required)
            }
        });
        return newArray;*/ // Return array generated by the above arrayFilter loop
    });
